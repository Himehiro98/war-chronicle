...
import { THINK_QUESTIONS } from '@/lib/think-questions';
...
/* HTMLから最初の一文を「文の途中で切らずに」抽出（先行オーガナイザー用）
   - 。があれば最初の文を丸ごと返す（長くても完結優先）
   - <br>区切りのリスト形式（actors等）は最初の項目を丸ごと返す */
function firstSentence(html: string | undefined, hardCap = 220): string {
  if (!html) return '';
  // <br>をセグメント区切りに変換してからタグ除去
  const segmented = html.replace(/<br\s*\/?>/gi, '\n');
  const plain = segmented.replace(/<[^>]*>/g, '').replace(/&[a-z]+;/g, ' ').replace(/[ \t]+/g, ' ').trim();
  // 最初のセグメント（<br>区切りの1項目）を取得
  const firstSeg = plain.split('\n')[0].trim();
  // セグメント内に。があれば最初の文で完結させる
  const idx = firstSeg.indexOf('。');
  const candidate = idx > 0 ? firstSeg.slice(0, idx + 1) : firstSeg;
  // 極端に長い場合のみ、読点で区切って完結させる
  if (candidate.length > hardCap) {
    const cut = candidate.lastIndexOf('、', hardCap);
    return cut > 40 ? candidate.slice(0, cut) + '——' : candidate.slice(0, hardCap) + '…';
  }
  return candidate;
}
...
function ThinkPrompt({ war, content }: { war: War; content: TabContent }) {
  ...
  // 戦争ごとの個別質問を最優先。無い場合のみタグ別テンプレートにフォールバック
  const individual = THINK_QUESTIONS[war.id];
  const tag = (war.tags ?? []).find(t => THINK_TEMPLATES[t]);
  const question = individual ?? (tag ? THINK_TEMPLATES[tag]! : THINK_FALLBACK);
  const hint = firstSentence(content.structure?.summary, 160);
  return (
